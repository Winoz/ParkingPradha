...
const Booking = {
  ...
  renderStep3() {
    ...
    // ========================================
    // FIX: Gunakan DISKON_ dan parse nilai %
    // ========================================
    const getDiscount = (m) => {
      const setting = s.settings.find(st => st.key === `DISKON_${m}`);
      if (!setting) return 0;
      // Parse nilai: "15%" -> 15, "5%" -> 5, "0" -> 0
      const val = setting.value.replace('%', '').trim();
      return Number(val) || 0;
    };
  ...
  calculatePrice() {
    ...
    // FIX: Gunakan DISKON_ dan parse nilai %
    // ========================================
    const discSetting = s.settings.find(st => st.key === `DISKON_${s.duration}`);
    let disc = 0;
    if (discSetting) {
      // Parse nilai: "15%" -> 15, "5%" -> 5, "0" -> 0
      const val = discSetting.value.replace('%', '').trim();
      disc = Number(val) || 0;
    }
